...
<div id="sim-dropdown-menu">
    ...
    <button type="button"
        ...
        data-sim-id="21919" data-sim-number="07888291038"
        data-iccid="89443042334117134260"
        data-orderdetail-id="59835"
        data-label="Your SIM" data-status="ACTIVE"
        data-billing-amount="1260.00" data-billing-days=""
...
class TestParseSims:
    ...
    def test_parses_the_real_dashboard_markup(self) -> None:
        ...
        assert len(sims) == 1
        sim = sims[0]
        assert sim.sim_meta_id == "21919"
        assert sim.order_detail_id == "59835"
        assert sim.sim_number == "07888291038"
        assert sim.iccid == "89443042334117134260"
    ...
    def test_decoy_button_is_ignored(self) -> None:
        ...
        sims = parse_sims(DASHBOARD_HTML)
        assert [sim.sim_meta_id for sim in sims] == ["21919"]
...
class TestAsyncGetUsage:
    ...
    async def test_success_on_trigger(self) -> None:
        ...
        assert params == {"order_detail_id": "59835", "sim_meta_id": "21919"}
    ...
    async def test_pending_then_success_polls_status_endpoint(self) -> None:
        ...
        assert status_url == f"{BASE_URL}{DATA_USAGE_STATUS_PATH}/21919"
